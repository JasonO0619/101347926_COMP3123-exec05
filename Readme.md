...
express.Router() is used in Express.js applicaions because it allows for router handling within the same applicaation which creates the opportunity for modular components. This benefits the code structure since it displays a more cleaner code, thus enhancing the readability.
...
I would implement a middleware function which would act upon an error found in a general aspect. My code in the index.js is an example where there's error handling which would target any error and provide the accurate message displayed:
...
the 'app.listen()' method starts the server as it connects to the port assigned, allowing requests to function. The 'process.env.port || 8081' basically determines the port that'll be connected, whether its the port from the environment variable or the default port. This is useful in avoiding any complications with other applicaitons running on the same server.
